/* Tyga Financials — execution controls + live demo balance sync. */
(() => {
  'use strict';
  ...
  function getBalance(){const e=ex(),a=window.accountSettings;const vals=[balance,e?.accountInfo?.balance,a?.balance,a?.accountBalance];for(const v of vals){const n=Number(v);if(Number.isFinite(n)&&n>0)return n}return NaN;}
  ...
  function paint(){
    ...
    const r=risk?.settings?.();const s=document.getElementById('tygaDemoStakeInfo');if(s){const b=getBalance();s.textContent=mode()==='PAPER'&&Number.isFinite(b)?`Demo max stake: ${(b*.60).toFixed(2)} (60% of ${b.toFixed(2)})`:'Live max stake: $10';}
  }
  function install(){
    patchRisk();
    if(typeof window.executeAutoTrade!=='function')return false;
    if(!window.executeAutoTrade.__tygaExecutionControl){const original=window.executeAutoTrade;window.executeAutoTrade=function(...args){if(!getEnabled()){try{window.logVerdict?.('AUTO EXECUTION OFF — signal only')}catch(_){}return false;}return original.apply(this,args)};window.executeAutoTrade.__tygaExecutionControl=true;}
    if(!document.getElementById('tygaExecutionToggle')){const a=document.createElement('button');a.id='tygaExecutionToggle';a.type='button';a.title='Arm/disarm automatic execution; hard safety limits remain active.';Object.assign(a.style,{position:'fixed',right:'12px',bottom:'132px',zIndex:10000,padding:'9px 12px',borderRadius:'11px',border:'1px solid var(--line,#362C5C)',background:'var(--panel,#161331)',font:'700 11px ui-monospace,monospace'});a.onclick=()=>{const next=!getEnabled();if(next&&!confirm('Arm automated execution? In LIVE mode this can send trades to the connected account.'))return;setEnabled(next);paint()};document.body.appendChild(a)}
    if(!document.getElementById('tygaModeToggle')){const m=document.createElement('button');m.id='tygaModeToggle';m.type='button';m.title='Switch PAPER/LIVE. Hard loss, position and cooldown limits remain active.';Object.assign(m.style,{position:'fixed',right:'12px',bottom:'174px',zIndex:10000,padding:'9px 12px',borderRadius:'11px',border:'1px solid var(--gold,#EAC069)',background:'var(--panel,#161331)',font:'700 11px ui-monospace,monospace'});m.onclick=()=>{if(!risk)return;if(mode()==='LIVE'){risk.configure({mode:'paper'});}else{if(!confirm('Enable LIVE mode? Real-money trades may be sent. Hard risk limits remain active.'))return;risk.configure({mode:'live'});}paint()};document.body.appendChild(m)}
    if(!document.getElementById('tygaDemoStakeInfo')){const s=document.createElement('div');s.id='tygaDemoStakeInfo';Object.assign(s.style,{position:'fixed',right:'12px',bottom:'216px',zIndex:9999,padding:'5px 8px',borderRadius:'8px',background:'rgba(10,16,29,.88)',color:'var(--muted,#9AAAC4)',font:'700 9px ui-monospace,monospace'});document.body.appendChild(s)}
    paint();return true;
  }
  const timer=setInterval(()=>{syncDemoBalance().finally(()=>{if(install()){};});},5000);install();
})();
